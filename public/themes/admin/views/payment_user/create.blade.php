    ...
    <div class="main_full">
        <div class="layui-col-md12">
            ...
            <div class="fb-main-table">
                <form class="layui-form" action="{{guard_url('payment_user')}}" method="post" lay-filter="fb-form">
                        ...
                        <div class="layui-input-inline">
                            <select name="payment_company_id">
                                @foreach($payment_companies as $key => $payment_company)
                                    <option value="{{ $payment_company->id }}" @if($payment_company->id == $payment_user->payment_company_id) selected @endif>{{ $payment_company->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="layui-form-item">
                        ...
                        <div class="layui-input-inline">
                            <input type="text" name="phone" value="{{ $payment_user->phone }}" lay-verify="required" autocomplete="off" placeholder="请输入{{ trans("payment_user.label.phone") }}" class="layui-input" >
                        </div>
                    </div>
                    ...
                    <div class="layui-form-item">
                        <label class="layui-form-label">{{ trans("payment_user.label.password") }}</label>
                        <div class="layui-input-inline">
                            <input type="password" name="password" placeholder="请输入{{ trans("payment_user.label.password") }}" autocomplete="off" class="layui-input"  lay-verify="required" >
                        </div>
