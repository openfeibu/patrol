...
class ProviderUserRepository extends BaseRepository implements ProviderUserRepositoryInterface
{
    ...
    {
        $this->fieldSearchable = config('model.user.provider_user.model.search');
    }
    ...
    {
        $provider = config("auth.guards." . getenv('guard') . ".provider", 'provider_users');
        return config("auth.providers.$provider.model", \App\Models\ProviderUser::class);
    }
