...
trait HasRoleAndPermission
{
    ...
    {
        return $this->belongsToMany(config('model.payment_roles.payment_role.model.model'),'payment_user_payment_role')->withTimestamps();
    }
    ...
    {
        $permissionModel = app(config('model.payment_roles.payment_permission.model.model'));

        if (!$permissionModel instanceof Model) {
            throw new InvalidArgumentException('[model.payment_roles.payment_permission.model.model] must be an instance of \Illuminate\Database\Eloquent\Model');
        }

        return $permissionModel::select(['payment_permissions.slug', 'payment_permissions.name'])
